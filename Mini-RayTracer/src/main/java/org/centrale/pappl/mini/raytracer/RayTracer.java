...
//        RayCastResult hitRayCastResult = null;
        ...
        Vector3 illumination = ray.getIllumination();

        raster.colorize(i, j, illumination);
...
/*        for (SceneObject sceneObject : Scene.getScene().getSceneObjects()) {
        ...
        if (closestSceneObject != null) {
            ...
            for (Light light : Scene.getScene().getLights()) {
                if (light instanceof DirectionalLight) {
                    ...
                    shadowDirection = new Vector3(((DirectionalLight) light).getDirection()).scale(-1).normalized();
                    ...
                    for (SceneObject sceneObject :
                        ...
                        shadowRCR = sceneObject.intersect(shadowRay);
                    }
                ...
                for (Light li : visibleLights) {
                    ...
                    if (li instanceof DirectionalLight) Lm = new Vector3(((DirectionalLight) li).getDirection()).scale(-1).normalized();
                    ...
                    illumination = illumination
                            .add(id.scale(kd).scale(Math.max(0, Lm.dot(N))))
            ...
            raster.colorize(i, j, illumination);
            System.out.println(visibleLights.size());
            visibleLights.clear();
        }*/
        ...
        if (closestSceneObject != null) {
            ...
            for (Light light : Scene.getScene().getLights()) {
                if (light instanceof DirectionalLight) {
                    double hitAngle = hitRayCastResult.normal.dot(((DirectionalLight) light).getDirection());
                    double intensity = -light.getIntensity();
                    Vector3 vector = light.getLightColor().scale(intensity * hitAngle);
